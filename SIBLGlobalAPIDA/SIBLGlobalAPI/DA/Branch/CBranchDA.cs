...
namespace SIBLGlobalAPI.DA.Branch
{
    public class CBranchDA
    {
        ...
        public CResult GetBranchList(CBranch oBranch)
        {
            ...
            try
            {
                using (IDbConnection oConnection = oDatabase.CreateConnection())
                {
                    IDbCommand oDbCommand = oDatabase.GetStoredProcICommand("PKG_REPORTS.GET_BRANCHINFO");
                    try
                    {
        ...
        private CBranchList BuildGetBranchList(DataTable dt)
        {
            CBranchList oBranchList = new CBranchList();
            CBranch oBranch = new CBranch();
            CBranch oBranchAdd = new CBranch();
            oBranchAdd.CN = "";
            oBranchAdd.BranchName = "Select Branch";
            oBranchList.BranchList.Add(oBranchAdd);            
        ...
        [Author("Md. Aminul Islam", "28-10-2017", "Get Branch Code")]
        public CResult GetBranchCode(CBranch oBranch)
        {
            CResult oResult = new CResult();
            Database oDatabase = new Database();
            DataSet oDataSet = null;
            ...
            try
            {
                using (IDbConnection oConnection = oDatabase.CreateConnection())
                {
                    IDbCommand oDbCommand = oDatabase.GetStoredProcICommand("PKG_TITAS.GetBrCodeByRoutingNo");

                    try
                    {
                        oDatabase.AddInOutParameter(oDbCommand, "p_BRANCHROUTINGNO", DbType.String, 200, oBranch.RoutingNumber);
                        oDatabase.AddInOutParameter(oDbCommand, "p_BrCode", DbType.String, 200, DBNull.Value);
                        oDatabase.AddInOutParameter(oDbCommand, "p_success", DbType.String, 50, DBNull.Value);
                        oDatabase.AddInOutParameter(oDbCommand, "p_return_msg", DbType.String, 500, DBNull.Value);
                        int i = oDatabase.ExecuteNonQuery(oDbCommand);
                        string success = ((OracleParameter)oDbCommand.Parameters[oDatabase.BuildParameterName("p_success")]).Value.ToString();
                        string sMessage = ((OracleParameter)oDbCommand.Parameters[oDatabase.BuildParameterName("p_return_msg")]).Value.ToString();
                        string branchCode = ((OracleParameter)oDbCommand.Parameters[oDatabase.BuildParameterName("p_BrCode")]).Value.ToString();
                        if (success == "1")
                        {
                            oResult.Result = true;
                            oBranch.BranchCode = branchCode;
                            oResult.Return = oBranch;
                        }
                        else
                        {
                            oResult.Result = false;
                    ...
                    {
                        CLog.Logger.Write(CLog.EXCEPTION, this.ToString() + "/GetBanchCode: " + exp.Message);
                        oResult.Exception = exp;
                        oResult.Message = exp.Message;
                        oResult.Result = false;
                        oResult.Return = null;
