                                    ...
                                    else
                                    {
                                        ...
                                        if (isIAvailable > 0)
                                        {
                                            ...
                                            if (DebitTxnReversal.ToUpper().Equals(DB_TXN_REVERSAL_YES))
                                            {
                                                ...
                                                sSql = "update TRANSACTIONINFO set PUSHRESPONSE ='', MGSTATUS='" + mgstatusCode + "',PUSHRESPONSEDATE='', TRASACTIONTYPE='BEFTN',FINALSTATUS='"+DB_TXN_PENDING+"', NPSBRESCODE='" + oTokenResponse.OutParameter.AuthResponseCode + "' where SIBLTRASNSACTIONID='" + oAccDeposit.SIBLtransactionNo + "'";
                                                oDbCommand.CommandText = sSql;
                                                oDbCommand.CommandType = CommandType.Text;
                                                iSuccess = oDbCommand.ExecuteNonQuery();
