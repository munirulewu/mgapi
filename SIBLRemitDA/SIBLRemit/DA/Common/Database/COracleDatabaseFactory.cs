...
namespace SIBLRemit.DA.Common.Connections
{
    /// <summary>
    ...
    {        
        ...
        protected override IDbConnection GetConnection(string sConnName)
        {
            IDbConnection oCnnection = null;
            try
            {

                string sConnectionString = GetConnectionStrings(sConnName);
                oCnnection = new OracleConnection(sConnectionString);
                return oCnnection;
            }
            catch (OracleException oEx)
        ...
        string GetConnectionStrings(string sConnectionName)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public override IDbConnection OpenConnection(string sConnName)
        {
            IDbConnection oOraConnection = null;      
            try
            {
                oOraConnection = (OracleConnection)GetConnection(sConnName);
                oOraConnection.Open();                
            }
            catch (OracleException oEx)
            {
